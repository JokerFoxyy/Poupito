...
**#42 — Correção de bugs reportados por usuários** ✅ CONCLUÍDA (2026-08-10 — SDD: `docs/session-42-correcao-bugs/SDD.md`; feedback direto de usuários registrado 2026-08-02)

Motivação: dois problemas reportados na Configurações/fluxo de conta, sem relação entre si.

1. **Apagar cartões e fixos → virou arquivar**: excluir cartão/fixo com transação/fatura/ocorrência vinculada sempre bloqueava (409 por FK, comportamento documentado desde a #25/#32). Em vez de só melhorar a mensagem, a solução definitiva foi **arquivar**: cartão/fixo some da tela principal e de qualquer seletor "Pagar com" (Transações/Fixos/Importer), mas transações/ocorrências/faturas **já existentes continuam 100% visíveis no histórico**.
   - Migration **V16**: `archived` boolean (default `false`, aditivo, sem backfill) em `cards` e `recurring_transactions` — **distinto** do `active` que `RecurringTransaction` já tinha (esse só pausa/retoma a materialização mensal, #8/#32; `archived` é sobre visibilidade). Fixo arquivado nunca materializa nova ocorrência, mesmo com `active=true` (checado no filtro do `RecurringMaterializationService`, não em `occursIn()` — esse método propositalmente não olha `archived`, pra continuar exibindo ocorrência já materializada antes do arquivamento).
   - `GET /v1/cards`/`GET /v1/recurring` ganham `?archived=` (default `false`); `PATCH .../{id}/archive` e `/unarchive` (endpoints dedicados). Histórico (`transactions`, `invoices`, `recurring/occurrences`) nunca filtra por archived. `DELETE` continua idêntico (409 se vínculo) — arquivar é a alternativa oferecida, não substitui a exclusão em cascata (que fica pra #27, agora só sobre Account).
   - Frontend: ação "Arquivar" sempre visível por linha (mais descobrível que só depois de um 409 falho) em `cards-panel`/`recurring`; seção "Arquivados" (`<details>`, classe `.archived-section` global) buscada à parte via `service.list(true)`, fora do store compartilhado.
2. **Vazamento de dados entre usuários no mesmo navegador** (logout→login trocando de conta, "Configurações" mostrava dados do usuário anterior) — bug de privacidade, prioridade alta. Causa-raiz: `AccountStore`/`CardStore`/`CategoryStore` (singletons `providedIn: 'root'`, sessão #26) tinham uma flag `loaded` nunca resetada; `Shell.logout()` só faz `router.navigate` sem reload de página, então nenhum singleton é recriado e `ensureLoaded()` nunca refazia o fetch pro próximo usuário. Fix: `reset()` em cada store, chamado por `AuthService.clearSession()` (único ponto que cobre logout explícito e falha silenciosa de refresh). Auditoria dos 17 serviços `providedIn: 'root'` do frontend confirmou que só esses 3 tinham o padrão de risco — `ThemeService` é intencionalmente por dispositivo (não deve resetar), `Shell.budgetAlertCount` é signal de componente (destruído no logout junto com o `Shell`, não é leak).

Testes: unitários (`CardServiceTest`/`RecurringServiceTest`/`RecurringMaterializationServiceTest`) + integração real via Testcontainers (`RecurringFlowIntegrationTest`, incluindo 409 de delete, archive/unarchive, materialização bloqueada e histórico preservado) — API JaCoCo mantido ≥90%. Web: 322/322 Karma, cobertura 97,06/87,14/93,86/96,98 (novos specs em `account/card/category.store.spec.ts`, `auth.service.spec.ts`, `cards-panel.spec.ts`, `recurring.spec.ts`, `card.service.spec.ts`, `recurring.service.spec.ts`).

Verificação e2e ao vivo (usuário de teste local + segundo usuário criado na hora): arquivar cartão com fatura vinculada → some do seletor "Pagar com" e da tela de Cartões, mas a transação e a fatura continuam 100% visíveis com o nome do cartão; desarquivar → volta a aparecer normalmente. Logout do usuário A → login do usuário B na mesma aba (sem reload) → Configurações mostra corretamente "nenhuma conta/categoria ainda", zero resquício dos dados de A. **Gotcha da verificação**: um processo Java do backend de uma sessão anterior sobreviveu a vários `docker compose down`/`up` do Postgres, ocupando a porta 8080 com código antigo (sem os campos `archived`) — mascarou o resultado até `jps -l` revelar o processo zumbi e ele ser encerrado.
